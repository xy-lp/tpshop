...
<form method="POST" action="/index.php/Admin/Role/add" name="theFrom">
    <div class="list-div">
        <table width="100%">
            <tbody><tr>
...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
        <td width="18%" valign="top" class="first-cell">
            <input name="action_code[]" type="checkbox" value="<?php echo ($vo["id"]); ?>" onclick="check('4,5',this);" class="checkbox"><?php echo ($vo["priv_name"]); ?></td>
        <td>
    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo1): $mod = ($i % 2 );++$i; if($vo['id'] == $vo1['parent_pid']): ?><volist name="child" id="vo2">
                <?php if($vo1['id'] == $vo2['parent_pid']): ?><div style="width:200px;float:left;">
                        <label for="<?php echo ($vo2["id"]); ?>"><input type="checkbox" name="action_code[]" value="<?php echo ($vo2["id"]); ?>" id="<?php echo ($vo2["id"]); ?>" class="checkbox" onclick="checkrelevance('', <?php echo ($vo1["id"]); ?>)" title="">
                            <?php echo ($vo1["priv_name"]); ?>/<?php echo ($vo2["priv_name"]); ?></label>
                    </div>
                <?php else: ?>
                    <div style="width:200px;float:left;">
                        <label for="<?php echo ($vo1["id"]); ?>"><input type="checkbox" name="action_code[]" value="<?php echo ($vo1["id"]); ?>" id="<?php echo ($vo1["id"]); ?>" class="checkbox" onclick="checkrelevance('',<?php echo ($vo1["id"]); ?>)" title="">
